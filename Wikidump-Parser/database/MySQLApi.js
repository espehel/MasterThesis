...
/*var insertCategory = function (name, pageId) {
    //console.log("2: ", name);
    connection.query('SELECT `id` FROM `categories` WHERE `name` = ?', [name], function(err, results, fields){
        if(err){
            console.log(err)
        } else {
            //if the category exists already
            if (results.length > 0) {
                ...
                connection.query("INSERT INTO `categories` SET ?", {name: name}, function (err, result) {
                    if(err){
                        console.log("`"+name+"`");
                        console.log(results);
                        console.log(err);
      ...
      //for(var i = 0; i<categories.length;i++) {
        ...
        connection.query('SELECT `id` FROM `categories` WHERE `name` = ?', [name], function(err, results, fields){
            if(err){
                ...
                if (results.length > 0) {
                    linkCategoryToPage(results[0].id, pageId);
                //if it does not we wait and try again, it may be inserted between theese lines
                } else {
                    //console.log("1: ", name);
                    setTimeout(function () {insertCategory(name,pageId)},2000);
                }
            }
        })
